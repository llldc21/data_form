...
function NovaSenha($email){
    ...
    if($res->num_rows >0){
      $s = $res->fetch_array();
      $pass = md5(date('h:m d/m/Y'));
      $sqll = 'UPDATE  TB_USUARIO SET DS_SENHA="'.$pass.'" WHERE DS_EMAIL="'.$email.'"';
      $ress = $GLOBALS['conn']->query($sqll);
      if($res){
           //EMAIL
    include_once("class.phpmailer.php");
    $destino = $email;
    $body = 'Sua senha temporária ->' .$pass;
    $subject = "Senha";
  ...
  $mail->setLanguage('pt'); //vc sabe (aqui no caso varia de acordo com o servidor, gmail por exemplo usa "br" não "pt" e não acusa erro, e tambem nao envia email)
  $mail->isSMTP(); //envio autenticado
  $mail->Host = 'mail.profrodolfo.com.br'; //geralmente, precisa ver no painel de controle do site qual é o endereço 
  $mail->SMTPAuth = true;
  $mail->SMTPSecure = 'tls'; //normalmente é c (gmail..etc), aqui deixei esse protocolo que era o indicado pelo servidor
  $mail->Username = 'aluno@profrodolfo.com.br'; // login 
  $mail->Password = '@alunoEtec'; // senha
